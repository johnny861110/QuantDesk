...
def _llm_synthesize_chip(
    ...
    try:
        ...
        prompt = f"""你是台股籌碼分析師。以下是 {symbol} 的籌碼面計算結果：

{findings_text}

系統訊號：{signal_label}（信心：{confidence:.0%}）

請用 2-3 句繁體中文說明籌碼面的**意涵**，重點放在：
- 法人（外資/投信/自營）的方向是否一致，趨勢延續幾日
- 融資槓桿是否有壓力，外資持股比例的變化方向
- 整體籌碼態勢對後市的解讀

【嚴格禁止】
- 禁止直接複讀數字（例：「外資淨股數為-80773006」「融資餘額31915」等原始數值）
- 禁止用「得分為X」「指標為Y」等格式複讀 key_findings
- 只說趨勢與意涵，不說原始數據

【格式要求】
- 直接給結論，不要有「根據以上資料」「整體而言」等廢話開頭
- 字數控制在 60-100 字
"""
