...
function handleEvent(
  ...
  switch (type) {
    ...
    case 'agent_error':
      return { ...state, currentEvent: `${payload.agent} 失敗: ${payload.error}` }
    ...
    case 'done':
      return { ...state, status: 'done', currentEvent: '分析完成 ✓' }
  ...
  const analyze = useCallback((query: string) => {
    ...
    es.onmessage = (e: MessageEvent) => {
      try {
        ...
        setState(prev => handleEvent(prev, type as string, payload))
